...
> The `saved` and `llm_authored` modes need the interpreter extra (`uv add "langclaw[interpreter]"`). Their bodies run in a capability-scoped sandbox limited to the tools you allow via `uses_tools` — no filesystem, network, or ambient host APIs.
