...
class TelegramChannel(BaseChannel):
    ...
    async def send_tool_progress(self, msg: OutboundMessage) -> None:
        ...
        ``tool_call_id`` — e.g. a workflow's ``▸ phase`` / ``⚙ step`` lines — has
        no result to pair with, so it is rendered immediately. (Previously such
        messages were silently dropped, hiding workflow execution from the chat.)
        """
